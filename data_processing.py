...
#and the bins categorized by 5 degrees
...
#since these functions taken out don't generate a correct x-axis for one of the figures
df['latitude_bin_2'] = pd.cut(df['latitude'], bins=np.arange(-30, 30, 5)) 

#This ensures that if there are any duplicate rows in the latitude_bin)2 df that they are dropped 
df.drop_duplicates(subset=['latitude_bin_2', 'coral_cover_2020', 'coral_cover_2100'], inplace=True)

#SThis categorizes SST_difference neatly to be used on the x-axis
...
#This chunk of code does the same as above but with the coral cover from 2100
...
plt.title('Average Coral Cover in 2020 and 2100') #This creates the title
#The code below creates a caption for the figure, placing it below the x-axis and determining the size
...
plt.show() #creates and shows the chart
